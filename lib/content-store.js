...
async function leerImagenes() {
  const { data, error } = await supabaseServiceClient().from('sitio_imagenes').select('slot, url, zoom_s, zoom_x, zoom_y');
...
/** Todo lo que guarda el editor visual, en la forma de GET /api/content. */
async function leerContenidoSitio() {
  const [data, { images, imageView }] = await Promise.all([readContent(), leerImagenes()]);
  return Object.assign({}, data, { images, imageView });
}

module.exports = { readContent, writeContent, updateContent, leerImagenes, leerContenidoSitio, CONTENT_PATH };
